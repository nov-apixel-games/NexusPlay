...
export default function DeveloperConsole({ userId, userProfile, onClose, onAddApp, publishedApps: initialApps }: DeveloperConsoleProps) {
  ...
  const menuItems = [
    { id: 'dashboard', label: t("dev.dashboard") || 'Consola Principal', icon: LayoutDashboard },
    { id: 'apps', label: t("dev.myApps") || 'Mis Aplicaciones', icon: Package },
    { id: 'publish', label: t("dev.publishApp") || 'Publicar Nueva App', icon: Upload, highlight: true },
    { id: 'analytics', label: t("dev.analytics") || 'Estadísticas', icon: Activity },
        ...
        <div className="p-8 pb-12 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[8px] font-black text-nexus-text/40 tracking-[0.3em] uppercase mt-1">{t("dev.subsystem") || "SISTEMA MULTI-NODAL"}</p>
            </div>
          </div>
          ...
          <div className="bg-nexus-card/50 rounded-3xl p-6 border border-nexus-border space-y-4">
             <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                   <p className="text-xs font-black text-nexus-text truncate uppercase tracking-tight">{userProfile?.username || t("dev.developer")}</p>
              ...
              <div className="h-20 shrink-0 border-b border-nexus-border flex items-center justify-between px-4 lg:px-12 bg-nexus-surface">
                ...
                <div className="flex items-center gap-4">
                   <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-nexus-card rounded-full border border-nexus-border">
                      ...
                      <span className="text-[10px] font-black text-nexus-text-sec uppercase tracking-widest">{t("dev.online") || "Online"}</span>
                   </div>
                  ...
                  >
                    {activeTab === 'dashboard' && (
                      <div className="space-y-8 lg:space-y-12">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
                          {[
                            { label: t("nav.apps") || 'Aplicaciones', value: stats.appCount, icon: Package, color: 'text-cyan-400', bg: 'bg-cyan-500/5' },
                            { label: t("dev.totalDownloads") || 'Descargas Totales', value: stats.totalDownloads, icon: Download, color: 'text-purple-400', bg: 'bg-purple-500/5' },
                            { label: t("dev.rating") || 'Rating', value: stats.avgRating, icon: Star, color: 'text-yellow-400', bg: 'bg-yellow-500/5' },
                          ].map((stat, idx) => (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                           <div className="lg:col-span-2 space-y-6">
                              <div className="flex items-center justify-between">
                                 <h3 className="text-xl font-black text-nexus-text uppercase tracking-tighter">{t("dev.latestReleases") || "Últimos Lanzamientos"}</h3>
                                 <button onClick={() => setActiveTab('apps')} className="text-[10px] font-black text-cyan-400 uppercase tracking-widest flex items-center gap-2 hover:gap-3 transition-all">
                                   {t("dev.seeAll") || "Ver todo"} <ArrowRight className="w-4 h-4" />
                                 </button>
                              </div>
                              ...
                              <div className="grid grid-cols-1 gap-4">
                                  ...
                                  <div key={app.id} className="p-4 lg:p-6 bg-nexus-card/50 border border-nexus-border rounded-3xl flex items-center gap-4 lg:gap-6 hover:bg-nexus-card/50 transition-all">
                                     ...
                                     <div className="text-right flex items-center gap-3 lg:gap-6 shrink-0">
                                        <div className="hidden sm:block">
                                           <p className="text-[10px] lg:text-xs font-black text-nexus-text whitespace-nowrap">{app.downloads} {t("dev.downloads") || "Descargas"}</p>
                                ...
                                {apps.length === 0 && (
                                  <div className="p-12 bg-nexus-card/50 border border-dashed border-nexus-border rounded-3xl text-center">
                                     ...
                                     <p className="text-nexus-text-sec font-bold uppercase text-xs">{t("dev.noApps") || "No hay aplicaciones publicadas aún."}</p>
                                     <button onClick={() => setActiveTab('publish')} className="mt-4 text-cyan-400 font-black uppercase text-[10px] hover:underline">{t("dev.startNow") || "Comenzar ahora"}</button>
                                  </div>
                                )}
                              </div>
                           </div>
                           ...
                           <div className="space-y-6">
                              <h3 className="text-xl font-black text-nexus-text uppercase tracking-tighter">{t("dev.devAccount") || "Cuenta Developer"}</h3>
                              <div className="p-6 lg:p-8 bg-gradient-to-br from-cyan-500/10 to-blue-600/10 border border-cyan-500/20 rounded-[2rem] lg:rounded-[2.5rem] space-y-6">
                                 <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                       <p className="text-[10px] font-black text-nexus-text uppercase tracking-widest">{t("dev.level") || "Nivel"}</p>
                                       <p className="text-lg font-black text-cyan-400 uppercase tracking-tighter">{t("dev.premium") || "Premium"}</p>
                                    </div>
                                 </div>
                                 <div className="space-y-3">
                                    <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                                       <span className="text-nexus-text-sec">{t("dev.reputation") || "Reputación"}</span>
                                       <span className="text-nexus-text">{t("dev.excellent") || "Excelente"}</span>
                                    </div>
                                 ...
                                 <ul className="space-y-3 pt-4 border-t border-nexus-border">
                                    <li className="flex items-center gap-3 text-[10px] font-bold text-nexus-text-sec">
                                       <Check className="w-4 h-4 text-cyan-500" /> {t("dev.apiKeyActive") || "API KEY ACTIVA"}
                                    </li>
                                    <li className="flex items-center gap-3 text-[10px] font-bold text-nexus-text-sec">
                                       <Check className="w-4 h-4 text-cyan-500" /> {t("dev.unlimited") || "ILIMITADO"}
                                    </li>
                                    <li className="flex items-center gap-3 text-[10px] font-bold text-nexus-text-sec">
                                       <Check className="w-4 h-4 text-cyan-500" /> {t("dev.vipSupport") || "SOPORTE VÍP"}
                                    </li>
                                 </ul>
                              </div>
                              ...
                              <div className="p-4 lg:p-6 bg-nexus-card rounded-3xl border border-nexus-border flex items-center justify-between">
                                 <div>
                                    <p className="text-[10px] font-black text-nexus-text-sec uppercase tracking-widest mb-1">{t("dev.feedback") || "Feedback"}</p>
                                    <p className="text-xs lg:text-sm font-black text-nexus-text">24 {t("dev.comments") || "Comentarios"}</p>
                                 </div>
                       ...
                       <div className="space-y-8">
                          <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between">
                             <div className="relative flex-1 max-w-md">
                                ...
                                <input 
                                  placeholder={t("dev.searchApp") || "BUSCAR APP..."} 
                             ...
                             <div className="flex gap-2">
                                <button className="flex-1 lg:flex-none px-4 lg:px-6 py-3 lg:py-4 bg-nexus-card border border-nexus-border rounded-2xl flex items-center justify-center gap-2 hover:bg-nexus-card-hover transition-all">
                                   ...
                                   <span className="text-[10px] font-black uppercase">{t("dev.filter") || "Filtrar"}</span>
                                </button>
                                <button onClick={() => setActiveTab('publish')} className="flex-[2] lg:flex-none px-6 lg:px-8 py-3 lg:py-4 bg-white text-nexus-bg rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-3 hover:bg-cyan-500 transition-all whitespace-nowrap">
                                   <Plus className="w-4 h-4 lg:w-5 lg:h-5" /> {t("dev.publish") || "PUBLICAR"}
                                </button>
                             </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                              ...
                              <div key={app.id} className="p-6 lg:p-8 bg-nexus-card/50 border border-nexus-border rounded-[2rem] lg:rounded-[2.5rem] group hover:border-nexus-border transition-all relative overflow-hidden">
                                 ...
                                 <div className="mt-6 lg:mt-8 space-y-4 relative z-10">
                                    ...
                                    <div className="grid grid-cols-3 gap-2 lg:gap-4 py-4 lg:py-6 border-y border-nexus-border">
                                       <div>
                                          <p className="text-[8px] lg:text-[10px] font-black text-nexus-text-sec uppercase mb-1">{t("dev.downloadsUpper") || "Downloads"}</p>
                                          <p className="text-base lg:text-lg font-black text-nexus-text">{app.downloads}</p>
                                       </div>
                                       <div>
                                          <p className="text-[8px] lg:text-[10px] font-black text-nexus-text-sec uppercase mb-1">{t("dev.rating") || "Rating"}</p>
                                       ...
                                       <div>
                                          <p className="text-[8px] lg:text-[10px] font-black text-nexus-text-sec uppercase mb-1">{t("dev.status") || "Status"}</p>
                                          <span className="text-[8px] lg:text-[9px] font-black text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full uppercase truncate block">{t("dev.published") || "Publicado"}</span>
                                       </div>
                                    </div>
                                    ...
                                    <button className="w-full py-4 bg-nexus-card hover:bg-white text-nexus-text-sec hover:text-nexus-bg rounded-2xl font-black uppercase text-[9px] lg:text-[10px] tracking-[0.2em] transition-all flex items-center justify-center gap-2">
                                       {t("dev.manage") || "GESTIONAR"} <ChevronRight className="w-4 h-4" />
                                    </button>
                                 </div>
                              </div>
                            ))}
                            {apps.length === 0 && (
                              <div className="col-span-full py-12 lg:py-24 text-center">
                                 ...
                                 <h3 className="text-xl lg:text-2xl font-black text-gray-600 uppercase">{t("dev.noActivity") || "Sin Actividad"}</h3>
                                 <p className="text-xs text-nexus-text-sec max-w-sm mx-auto mt-2 px-4">{t("dev.noAppsDesc") || "No has publicado ninguna aplicación todavía. Comienza subiendo tu primer APK."}</p>
                              </div>
                            )}
                          </div>
                       </div>
                    ...
                    {activeTab === 'settings' && (
                       <div className="py-12 lg:py-24 flex flex-col items-center justify-center text-center space-y-4 px-4">
                          ...
                          <h3 className="text-2xl lg:text-3xl font-black uppercase text-gray-700 leading-tight">{t("nav.settings") || "Configuración"}</h3>
                          <p className="text-xs lg:text-sm text-nexus-text-sec max-w-md">{t("dev.settingsDevInfo") || "Opciones avanzadas en desarrollo."}</p>
                       </div>
                    )}
                  </motion.div>
                </AnimatePresence>
